...
async def handle_request(reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
    try:
        ...
        for ln in lines[1:]:
            if ln.lower().startswith("content-length:"):
                try:
                    ...
                    content_length = 0
                break
        _MAX_BODY = 262144  # 256 KB — guard against memory abuse
        too_large = content_length > _MAX_BODY
        body_bytes = rest
        if not too_large:
        ...
        for ln in lines[1:]:
            low = ln.lower()
            if low.startswith("x-forwarded-for:"):
                client_ip = ln.split(":", 1)[1].strip().split(",")[0].strip()
            elif low.startswith("x-admin-key:"):
                admin_key = ln.split(":", 1)[1].strip()
        ...
        try:
            if too_large:
                status, body = 413, _json_bytes({"ok": False, "error": "payload_too_large"})
                ...
                _admin = os.environ.get("EMAIL_ADMIN_KEY", "")
                if not _admin or admin_key != _admin:
                    status, body = 403, _json_bytes({"ok": False, "error": "forbidden"})
                else:
                    try:
                        payload = json.loads(body_bytes.decode("utf-8")) if body_bytes else {}
                    except (json.JSONDecodeError, UnicodeDecodeError):
        ...
        status_text = {200: "OK", 302: "Found", 400: "Bad Request", 403: "Forbidden", 404: "Not Found", 413: "Payload Too Large", 500: "Internal Server Error"}.get(status, "OK")
