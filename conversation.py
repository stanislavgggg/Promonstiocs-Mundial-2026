"""
conversation.py — email-capture mode (multibrand, CRO-optimised)
...
_SUCCESS = {
    ...
    "ru": (
        "✅ *Готово!*\n\n"
        "Проверь почту — первое письмо уже в пути.\n"
        "Отписаться можно в любой момент по ссылке в письме."
    ),
    "es": (
        "✅ *¡Ya estás dentro!*\n\n"
        "Revisa tu correo — el primer mensaje va en camino.\n"
        "Puedes darte de baja en cualquier momento desde el enlace de cualquier correo."
    ),
}
_ERR = {
    "en": "⚠️ Something went wrong — tap the button and try again.",
    "ru": "⚠️ Что-то пошло не так — нажми кнопку и попробуй снова.",
    "es": "⚠️ Algo salió mal — toca el botón e inténtalo de nuevo.",
}
_BAD_EMAIL = {
    "en": "⚠️ That email doesn't look right — please check and try again.",
    "ru": "⚠️ Email выглядит неправильно — проверь и попробуй снова.",
    "es": "⚠️ Ese correo no parece correcto — verifícalo e inténtalo de nuevo.",
}
_GEO_BLOCK = {
