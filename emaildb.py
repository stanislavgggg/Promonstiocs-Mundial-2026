...
def _pg_init():
    ...
    with _pg() as c:
        ...
        # Лиды (открыл бота, но мог ещё не оставить email) — для ретеншн-пушей.
        # Persistent в Postgres → переживают редеплой (в отличие от storage.py JSON).
        c.execute("""
        CREATE TABLE IF NOT EXISTS email_leads (
            ...
            last_push   DOUBLE PRECISION,
            UNIQUE (brand, tg_id)
        );""")
        c.execute("CREATE INDEX IF NOT EXISTS ix_leads_due ON email_leads (brand, converted, blocked);")
    _pg_ready = True
...
# ── leads (для ретеншн-пушей) ────────────────────────────────────────────────
def _leads():
    return _json_db.setdefault("leads", {})

def _lead_key(brand, tg_id):
    return f"{brand}:{tg_id}"

def touch_lead(brand, tg_id, lang=None, source=None):
    """Upsert лида (открыл бота / мини-апп). Не трогает converted/pushes у существующих."""
    if not tg_id:
        return
    tg_id = int(tg_id); now = _now()
    if _USE_PG:
        _pg_init()
        with _pg() as c:
            c.execute("""INSERT INTO email_leads (brand,tg_id,lang,source,first_seen,last_seen)
                         VALUES (%s,%s,%s,%s,%s,%s)
                         ON CONFLICT (brand,tg_id) DO UPDATE
                         SET last_seen=%s, lang=COALESCE(EXCLUDED.lang, email_leads.lang)""",
                      (brand, tg_id, lang, source, now, now, now))
    else:
        leads = _leads(); k = _lead_key(brand, tg_id)
        rec = leads.get(k)
        if rec:
            rec["last_seen"] = now
            if lang: rec["lang"] = lang
        else:
            leads[k] = dict(brand=brand, tg_id=tg_id, lang=lang, source=source,
                            converted=False, blocked=False, pushes=0,
                            first_seen=now, last_seen=now, last_push=None)
        _json_save()

def mark_lead_converted(brand, tg_id):
    if not tg_id:
        return
    tg_id = int(tg_id)
    if _USE_PG:
        _pg_init()
        with _pg() as c:
            c.execute("UPDATE email_leads SET converted=TRUE WHERE brand=%s AND tg_id=%s",
                      (brand, tg_id))
    else:
        rec = _leads().get(_lead_key(brand, tg_id))
        if rec: rec["converted"] = True; _json_save()

def mark_lead_blocked(brand, tg_id):
    """Юзер заблокировал бота — больше не пушим."""
    if not tg_id:
        return
    tg_id = int(tg_id)
    if _USE_PG:
        _pg_init()
        with _pg() as c:
            c.execute("UPDATE email_leads SET blocked=TRUE WHERE brand=%s AND tg_id=%s",
                      (brand, tg_id))
    else:
        rec = _leads().get(_lead_key(brand, tg_id))
        if rec: rec["blocked"] = True; _json_save()

def record_lead_push(brand, tg_id):
    tg_id = int(tg_id); now = _now()
    if _USE_PG:
        _pg_init()
        with _pg() as c:
            c.execute("UPDATE email_leads SET pushes=pushes+1, last_push=%s "
                      "WHERE brand=%s AND tg_id=%s", (now, brand, tg_id))
    else:
        rec = _leads().get(_lead_key(brand, tg_id))
        if rec: rec["pushes"] = rec.get("pushes", 0) + 1; rec["last_push"] = now; _json_save()

def leads_due_for_push(brand, min_age_s, gap_s, max_pushes, limit=500):
    """Лиды, которым пора слать ретеншн-пуш: не сконвертились, не заблокировали бота,
    появились давно (>= min_age_s), пушей < max_pushes, и с прошлого пуша прошло >= gap_s."""
    now = _now()
    if _USE_PG:
        _pg_init()
        with _pg() as c:
            rows = c.execute(
                """SELECT tg_id, lang, pushes FROM email_leads
                   WHERE brand=%s AND converted=FALSE AND blocked=FALSE
                     AND pushes < %s
                     AND first_seen <= %s
                     AND (last_push IS NULL OR last_push <= %s)
                   ORDER BY first_seen ASC LIMIT %s""",
                (brand, max_pushes, now - min_age_s, now - gap_s, limit)).fetchall()
            return [{"tg_id": r[0], "lang": r[1], "pushes": r[2]} for r in rows]
    out = []
    for rec in _leads().values():
        if rec.get("brand") != brand or rec.get("converted") or rec.get("blocked"):
            continue
        if rec.get("pushes", 0) >= max_pushes:
            continue
        if (rec.get("first_seen") or 0) > now - min_age_s:
            continue
        lp = rec.get("last_push")
        if lp and lp > now - gap_s:
            continue
        out.append({"tg_id": rec["tg_id"], "lang": rec.get("lang"), "pushes": rec.get("pushes", 0)})
    return out[:limit]

def lead_counts(brand=None):
    if _USE_PG:
        _pg_init()
        with _pg() as c:
            q = "SELECT COUNT(*), COUNT(*) FILTER (WHERE converted), COUNT(*) FILTER (WHERE blocked) FROM email_leads"
            args = ()
            if brand: q += " WHERE brand=%s"; args = (brand,)
            row = c.execute(q, args).fetchone()
            return {"leads": row[0], "converted": row[1], "blocked": row[2]}
    vals = [r for r in _leads().values() if not brand or r.get("brand") == brand]
    return {"leads": len(vals),
            "converted": sum(1 for r in vals if r.get("converted")),
            "blocked": sum(1 for r in vals if r.get("blocked"))}
