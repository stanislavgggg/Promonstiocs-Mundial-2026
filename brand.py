...
GOALCAST = Brand(
    ...
    character=Character(
        name="Diego",
        role="analista de fútbol",
        persona=(
            "Eres {name} — {role} de {brand}. "
            "Tono: cercano y basado en datos (xG, forma, lesiones), sin hype. "
            "CUMPLIMIENTO: solo análisis informativo del Mundial 2026; 18+; "
            "no prometes resultados ni beneficios y no incitas a apostar."
        ),
        honest_stats=True,   # nunca mostrar win-rate inflado
    ),
...
#: Второй футбольный бренд под Mundial 2026 — концепт «данные/аналитика» (скрин 1).
#  Точка захвата — Telegram Mini App (страница на Lovable), бот её открывает.
#  Сегмент — gambling (беттинг-типсы), поэтому на деплое: EMAIL_HARD_VERTICALS с football,
#  ESP_HARD = iGaming-ESP (НЕ Mailchimp), гео — allow_list по выверенным странам LATAM.
FUTBOLPLUS = Brand(
    id="futbolplus",
    display_name="fútbolplus",
    bot_username="FutbolPlusBot",
    tagline={
        "en": "World Cup 2026 football reads, in your inbox.",
        "es": "Análisis del Mundial 2026, en tu bandeja.",
    },
    character=Character(
        name="Leo",
        role="analista de datos de fútbol",
        persona=(
            "Eres {name} — {role} de {brand}. "
            "Tono: claro y basado en datos (xG, forma, lesiones), sin hype. "
            "CUMPLIMIENTO: solo análisis informativo del Mundial 2026; 18+; "
            "no prometes resultados ni beneficios y no incitas a apostar. "
            "Tu objetivo es que la gente reciba el análisis por email."
        ),
        honest_stats=True,
    ),
    sport=SportConfig(vertical=Vertical.FOOTBALL),
    ...
    cta=CTA(
        ...
        channel_handle="@your_channel",
        gate=False,
        button_label={"en": "Open the app", "es": "Abrir la app"},
    ),
    funnel=Funnel(repeat_enabled=False),
    privacy_url="",  # set per-deploy via PRIVACY_URL env
    copy_pack="copy_goalcast",  # переиспользуем футбольный копирайт; маркетинг живёт в мини-аппе
)
