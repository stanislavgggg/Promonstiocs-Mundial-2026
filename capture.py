...
Flow (single opt-in):
  subscribe(payload)  → validate + consent → store + mark confirmed → push to ESP
                        (also marks the Telegram lead converted, if tg_id present)
  confirm(token)      → legacy double-opt-in path (kept for compatibility)
...
async def subscribe(payload: dict, ip: str = "") -> tuple[int, dict]:
    ...
    # Они оставили email → лид сконвертился, ретеншн-пуши ему больше не нужны.
    if tg_id:
        try:
            emaildb.mark_lead_converted(emailcfg.BRAND_ID, tg_id)
        except Exception as e:
            logger.warning(f"mark_lead_converted failed: {e}")
