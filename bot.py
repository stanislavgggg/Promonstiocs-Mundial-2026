...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Регистрируем лида (persistent в Postgres) — основа для ретеншн-пушей.
    try:
        emaildb.touch_lead(BRAND.id, user.id, lang, "bot_start")
    except Exception as e:
        logger.warning(f"touch_lead failed: {e}")
...
async def post_init(application: Application):
    ...
    # Persistent menu button → opens the Mini App (the Lovable capture page).
    _miniapp = os.environ.get("MINI_APP_URL", "").rstrip("/")
    if _miniapp.startswith("https://"):
        try:
            await application.bot.set_chat_menu_button(
                menu_button=MenuButtonWebApp(
                    text="Abrir la app", web_app=WebAppInfo(url=_miniapp)))
            logger.info(f"menu button → mini app {_miniapp}")
        except Exception as e:
